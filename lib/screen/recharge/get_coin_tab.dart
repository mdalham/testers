...
int get _kCooldownSeconds => PublishConstants.adsCoolDown;
...
class _GetCoinTabState extends State<GetCoinTab> {
  ...
  void _startCooldown(
    ...
    final int seconds = initial ?? _kCooldownSeconds;
    ...
    setState(() {
      if (m == 1) {
        _m1Cooldown = true;
        _m1Secs = seconds;
        _m1WasCompletion = wasCompletion;
      } else if (m == 2) {
        _m2Cooldown = true;
        _m2Secs = seconds;
        _m2WasCompletion = wasCompletion;
      } else {
        _m3Cooldown = true;
        _m3Secs = seconds;
        _m3WasCompletion = wasCompletion;
      }
    });
  ...
  Future<void> _resetMilestone(int m) async {
    ...
    if (m == 1) {
      ...
      if (mounted)
        setState(() {
          _milestone1Done = false;
          _m1WasCompletion = false;
        });
    } else if (m == 2) {
      ...
      if (mounted)
        setState(() {
          _milestone2Progress = 0;
          _m2WasCompletion = false;
        });
    } else {
      ...
      if (mounted)
        setState(() {
          _milestone3Progress = 0;
          _m3WasCompletion = false;
        });
    }
  }
  ...
  Future<void> _onWatchAdMilestone2() async {
    ...
    await UnityAdsService.instance.showRewardedAd(
      onCompleted: () async {
        ...
        setState(() {
          _milestone2Progress = next;
          _m2AdPending = false;
        });
        final bool justCompleted = next == 4;
        _startCooldown(2, wasCompletion: justCompleted);
        if (justCompleted)
          _claimReward(PublishConstants.adsM2, CoinTxType.adReward);
      },
      onFailed: () {
        if (!mounted) return;
  ...
  Future<void> _onWatchAdMilestone3() async {
    ...
    await UnityAdsService.instance.showRewardedAd(
      onCompleted: () async {
        ...
        setState(() {
          _milestone3Progress = next;
          _m3AdPending = false;
        });
        final bool justCompleted = next == 10;
        _startCooldown(3, wasCompletion: justCompleted);
        if (justCompleted)
          _claimReward(PublishConstants.adsM3, CoinTxType.adReward);
      },
      onFailed: () {
        if (!mounted) return;
