...
import 'package:testers/screen/open_testers/publish_app/step1_app_setup.dart';
import 'package:testers/screen/open_testers/publish_app/step2_terms_availability.dart';
import 'package:testers/screen/open_testers/publish_app/step3_app_details.dart';
import 'package:testers/screen/open_testers/publish_app/step4_tester_selection.dart';
import 'package:testers/screen/open_testers/publish_app/step5_success.dart';
import '../../../controllers/height_width.dart';
import '../../../controllers/info.dart';
import '../../../service/ads/unity_interstitial_service.dart';
...
import '../../../widget/badge/coin_badge.dart';
...
class _CreatePublishFlowScreenState extends State<AppListingScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // ✅ clear stale data when opening a fresh publish flow
  ...
  Future<void> _triggerAction() async {
    ...
    // ✅ publish: check coin balance upfront (cost is known synchronously)
    if (widget.mode == PublishFlowMode.publish) {
    ...
    // ✅ republish: cost depends on storedMaxTesters — checked inside _doRepublish()
  ...
  Future<void> _doPublish() async {
    ...
    // ✅ dynamic cost: selectedTesterCount × kCoinsPerTester, discounted
    ...
    final ok = await p.publishApp(
      ...
      coinCost:      coinCost,           // ✅ no publishType param
    ...
    if (ok) {
      await CoinTransactionService.instance.logTransaction(
        ...
        amount:   -coinCost,             // ✅ matches exactly what was deducted
  ...
  Future<void> _doEdit() async {
    ...
    final ok = await p.updateApp(
      ...
      description:   p.step3Description.trim().isEmpty
          ...
          : p.step3Description.trim(),
      isRepublish: false,
      coinCost:    PublishConstants.editCoinCost, // ✅ fixed cost
    );
    if (!mounted) return;
    if (ok) {
      await CoinTransactionService.instance.logTransaction(
  ...
  Future<void> _doRepublish() async {
    ...
    // ✅ fetch storedMaxTesters from Firestore to compute the real cost
    ...
    // ✅ republishCost = storedMaxTesters × kCoinsPerTester, with discount
    ...
    final ok = await p.updateApp(
      ...
      description:   p.step3Description.trim().isEmpty
          ...
          : p.step3Description.trim(),
      isRepublish:   true,
      coinCost:      coinCost,           // ✅ discounted dynamic cost
      newMaxTesters: storedMaxTesters,   // ✅ preserve original tester count
    );
    if (!mounted) return;
    if (ok) {
      await CoinTransactionService.instance.logTransaction(
