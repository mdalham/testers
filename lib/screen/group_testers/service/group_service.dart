...
class ActiveGroupState {
  ...
  factory ActiveGroupState.fromMap(Map<String, dynamic> map) {
  ...
  static final empty = ActiveGroupState(
    activeCount:    0,
    completedCount: 0,
    formingCount:   0,
    totalCount:     0,
...
class GroupService {
  ...
  Stream<List<GroupModel>> watchAllGroups() => _col
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map((s) => s.docs.map(GroupModel.fromDoc).toList());

  Stream<ActiveGroupState> watchGroupStats() => _statsDoc.snapshots().map(
        (doc) => doc.exists
        ? ActiveGroupState.fromMap(doc.data() as Map<String, dynamic>)
        : ActiveGroupState.empty,
  );

  Stream<List<GroupModel>> watchActiveGroups() => _col
      .where('status', isEqualTo: GroupStatus.active.name)
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map((s) => s.docs.map(GroupModel.fromDoc).toList());

  Stream<List<GroupModel>> watchCompletedGroups() => _col
      .where('status', isEqualTo: GroupStatus.completed.name)
      .orderBy('createdAt', descending: true)
      .snapshots()
      .map((s) => s.docs.map(GroupModel.fromDoc).toList());

  Stream<GroupModel?> watchGroup(String id) => _col
      .doc(id)
      .snapshots()
      .map((d) => d.exists ? GroupModel.fromDoc(d) : null);

  Stream<GroupModel?> watchOpenFormingGroup() => _col
      .where('status', isEqualTo: 'forming')
      .limit(1)
      .snapshots()
      .map((s) => s.docs.isNotEmpty ? GroupModel.fromDoc(s.docs.first) : null);

  Future<void> _syncGroupStats() async {
    try {
      final snap      = await _col.get();
      final all       = snap.docs.map(GroupModel.fromDoc).toList();
      final active    = all.where((g) => g.status == GroupStatus.active).length;
      final completed = all.where((g) => g.status == GroupStatus.completed).length;
      final forming   = all.where((g) => g.status == GroupStatus.forming).length;
      ...
      debugPrint('[GroupService] _syncGroupStats ERROR: $e');
    }
  }
  ...
  Future<void> checkAndStartGroups() async {
    try {
      ...
      debugPrint('[GroupService] checkAndStartGroups ERROR: $e');
    }
  }

  bool _meetsStartCondition(GroupModel group) {
    if (group.members.length >= group.maxMembers) return true;
    final enoughMembers = group.members.length >= PublishConstants.miniGroupMembers;
  ...
  Future<void> _activateGroup(String groupId) async {
    List<GroupMember> membersSnapshot = [];
    ...
    await _db.runTransaction((tx) async {
      ...
      final group = GroupModel.fromDoc(snap);
      if (group.status != GroupStatus.forming) return;
  ...
  Future<void> _ensureFormingGroupExists() async {
    final snap = await _col.where('status', isEqualTo: 'forming').limit(1).get();
  ...
  Future<void> _createBlankFormingGroup() async {
    try {
      ...
      await _col.doc(uniqueId).set({
        'uniqueId':      uniqueId,
        'status':        GroupStatus.forming.name,
        'members':       [],
        'apps':          {},
      ...
      debugPrint('[GroupService] _createBlankFormingGroup ERROR: $e');
      rethrow;
    }
  }
  ...
  Future<String?> joinGroup({
    ...
    try {
      final ref = _col.doc(groupId);
      List<GroupMember> existingMembersSnapshot = [];
      ...
      await _db.runTransaction((tx) async {
        ...
        if (group.status != GroupStatus.forming)      throw 'This group is no longer accepting members.';

        existingMembersSnapshot = List.from(group.members);

        final newMember      = GroupMember(uid: uid, username: username);
        ...
        if (newCount >= group.maxMembers) {
          updates['status']        = GroupStatus.active.name;
      ...
      if (groupActivated) {
        await _notif.onGroupStarted(
          members: [...existingMembersSnapshot, GroupMember(uid: uid, username: username)],
          groupId: groupId,
        );
      }
      ...
      debugPrint('[GroupService.joinGroup] $e');
      return e.toString();
    }
  }
  ...
  Future<String?> createGroup({
    ...
    try {
      final existing = await _col.where('status', isEqualTo: 'forming').limit(1).get();
      ...
      final uniqueId = await _generateUniqueId();
      final member   = GroupMember(uid: uid, username: username);
      await _col.doc(uniqueId).set({
        'uniqueId':      uniqueId,
        'status':        GroupStatus.forming.name,
  ...
  Future<String?> uploadProof({
    ...
    try {
      final now       = DateTime.now();
      final dayNumber = (now.difference(taskStartDate).inHours ~/ 24 + 1).clamp(1, 14);
      final dayKey    = 'day-$dayNumber';

      final dayWindowEnd       = taskStartDate.add(Duration(days: dayNumber));
      final representativeDate = DateTime(
        dayWindowEnd.year, dayWindowEnd.month, dayWindowEnd.day,
      );
      ...
      final proofData = {
        ...
        if (reportText != null && reportText.isNotEmpty) 'reportText': reportText.trim(),
      };

      await _testedCol.doc(groupId).set(
        {'groupId': groupId, dayKey: {'${uid}_$targetUserId': proofData}},
        SetOptions(merge: true),
      );
      ...
      if (resolvedAppName.isEmpty) {
        try {
          ...
          if (groupDoc.exists) {
            final g       = GroupModel.fromDoc(groupDoc);
      ...
      debugPrint('[GroupService] uploadProof ERROR: $e');
      return e.toString();
    }
  }
  ...
  Future<ProofStatus> getTodayProofStatus({
    ...
    try {
      ...
      if (dayMap == null || !dayMap.containsKey(entryKey)) return ProofStatus.none;
      ...
      debugPrint('[GroupService] getTodayProofStatus ERROR: $e');
  ...
  Future<String?> approveTask({
    ...
    try {
      final dayNumber = (DateTime.now().difference(taskStartDate).inHours ~/ 24 + 1)
          .clamp(1, 14);
      final dayKey = 'day-$dayNumber';
      ...
      debugPrint('[GroupService] approveTask ERROR: $e');
      return e.toString();
    }
  }
  ...
  Future<String?> rejectTask({
    ...
    try {
      ...
      debugPrint('[GroupService] rejectTask ERROR: $e');
      return e.toString();
    }
  }
  ...
  Future<void> checkAndCompleteGroups() async {
    try {
      ...
      debugPrint('[GroupService] checkAndCompleteGroups ERROR: $e');
    }
  }
  ...
  Future<void> checkAndCompleteGroup(String groupId) async {
    try {
      ...
      debugPrint('[GroupService] checkAndCompleteGroup ERROR: $e');
    }
  }
  ...
  Future<String?> completeGroup(String groupId) async {
    try {
      ...
      final group = GroupModel.fromDoc(doc);
      if (group.status == GroupStatus.completed) return null;

      await _markGroupCompleted(groupId,
          members: group.members, isManuallyClosed: true);
      await _syncGroupStats();
      return null;
    } catch (e) {
      debugPrint('[GroupService] completeGroup ERROR: $e');
      return e.toString();
    }
  }
  ...
  Future<String?> removeMember({
    ...
    try {
      ...
      await _notif.onUserRemovedFromGroup(
          removedUid: memberUid, groupId: groupId);
      return null;
    } catch (e) {
      debugPrint('[GroupService] removeMember ERROR: $e');
      return e.toString();
    }
  }
  ...
  Future<void> sendDailyReminders({
    ...
    try {
      ...
      final submittedUids = dayMap.entries
          .map((e) => (e.value as Map<String, dynamic>?)?['uid'] as String? ?? '')
          .toSet();
      ...
      debugPrint('[GroupService] sendDailyReminders ERROR: $e');
    }
  }

  bool _isGroupExpired(GroupModel group) {
  ...
  Future<void> _markGroupCompleted(
    ...
    try {
      await _col.doc(groupId).update({
        'status':      GroupStatus.completed.name,
      ...
      debugPrint('[GroupService] _markGroupCompleted ERROR: $e');
    }
  }
  ...
  Future<void> autoApproveStaleTasks({
    ...
    try {
      final now        = DateTime.now();
      final currentDay = (now.difference(taskStartDate).inHours ~/ 24 + 1).clamp(1, 14);
      ...
      debugPrint('[GroupService] autoApproveStaleTasks ERROR: $e');
    }
  }
}