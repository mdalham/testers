...
import 'package:testers/screen/group_testers/widget/inactive_groups_section.dart';
...
import 'widget/group_members_grid.dart';
import 'widget/pending_install_tile.dart';

const _deepBlue = Color(0xFF1A237E);
const _orange = Color(0xFFFF9800);

// ─────────────────────────────────────────────────────────────────────────────
// InactiveGroupScreen  —  auto-rebuilds when any group state changes
// ─────────────────────────────────────────────────────────────────────────────

class InactiveGroupScreen extends StatefulWidget {
  const InactiveGroupScreen({
    super.key,
    required this.uid,
  ...
  @override
  State<InactiveGroupScreen> createState() => _InactiveGroupScreenState();
}

class _InactiveGroupScreenState extends State<InactiveGroupScreen> {
  StreamSubscription<ActiveGroupState>? _statsSub;
  StreamSubscription<GroupModel?>? _formingSub;

  GroupModel? _formingGroup;

  @override
  void initState() {
    super.initState();
    _formingSub = GroupService.instance.watchOpenFormingGroup().listen((group) {
      if (mounted) setState(() => _formingGroup = group);
    });

    _statsSub = GroupService.instance.watchGroupStats().listen((_) {
      if (mounted) setState(() {});
    });
  }
  ...
  List<PendingInstallItem> _buildPendingItems(GroupModel group) {
