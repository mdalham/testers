...
class _TestingSheetState extends State<_TestingSheet>
  ...
  // ── Add inside _TestingSheetState ──────────────────────────────────────────

  /// Returns true if the cooldown has elapsed (or never been set).
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Submit  —  passes taskStartDate so GroupService can compute the day key
  // ─────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _submit() async {
    ...
    if (adReady && cooldownOk) {
      ...
      UnityInterstitialService.instance.showInterstitialAd(
        ...
        onFailed: () async {
          // Ad failed — submit anyway so the user isn't blocked
          await _runSubmitUpload();
        },
      );
    } else {
      // Cooldown active OR no ad loaded — skip ad silently
      await _runSubmitUpload();
    }
  }

// Extracted Firestore upload so _submit stays readable
  Future<void> _runSubmitUpload() async {
    final error = await GroupService.instance.uploadProof(
