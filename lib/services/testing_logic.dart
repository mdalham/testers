...
import 'firebase/CoinTransaction/coin_transaction_service.dart';
    ...
    try {
      await db.runTransaction((txn) async {
        ...
        if (userTestedSnap.exists) {
          final testedApps =
              (userTestedSnap.data()?['testedApps'] as List<dynamic>?) ?? [];
        ...
        final maxTesters =
            (appSnap.data()?['maxTesters'] as num?)?.toInt() ?? 12;
        final current =
            (appSnap.data()?['currentTesterCount'] as num?)?.toInt() ?? 0;
